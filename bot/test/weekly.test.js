...
test('automatic release modes keep hotfix and weekly policies narrow', () => {
  ...
  assert.equal(weeklyInternals.autoReleaseAllowed({ ...base, latest: '0.5.0' }, 'weekly'), true);
  assert.equal(weeklyInternals.autoReleaseAllowed({ ...base, latest: '0.4.1' }, 'weekly'), false);
  assert.equal(weeklyInternals.autoReleaseAllowed({ ...base, latest: '0.4.1' }, 'daily'), true);
  assert.equal(weeklyInternals.autoReleaseAllowed({ ...base, channel: 'beta', latest: '0.5.0' }, 'weekly'), false);
});

test('beta stream can be checked daily without widening weekly automation', () => {
  const beta = {
...
test('automatic schedules use the configured local hour and a bounded window', () => {
  ...
  assert.equal(weeklyInternals.isAutoWindow(atFour, 'weekly', 4), false);
